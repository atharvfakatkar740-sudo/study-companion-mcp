...
export function exportProgress(userName) {
    ...
    const milestones = loadJSON("milestones-completed.json", []);
    ...
    const snapshot = {
        ...
        highlights: {
            ...
            recentInsights: memories
                ...
                .slice(-5)
                .map((m) => ({ content: m.content.slice(0, 100), topic: m.topic })),
...
export function compareProgress(otherUserName) {
    // Load own progress
    const own = exportProgress("me");
    ...
    const comparison = {
        ...
        sharedTopics: target.progress.completedTopics.filter((t) => own.completedTopics?.includes(t)),
    };
...
## Milestones Achieved
${(milestones || []).map((m) => `- ✅ ${m.name || m.milestone || m.id}`).join("\n") || "- None yet — keep going!"}

## Key Insights
${memories
        .filter((m) => m.type === "breakthrough" || m.type === "insight")
        .slice(-5)
        .map((m) => `- 💡 **${m.topic}**: ${m.content.slice(0, 80)}...`)
        .join("\n") || "- Start saving insights to build your knowledge base!"}

## Papers Read
${completedPapers.slice(-5).map((p) => `- 📄 ${p.title}`).join("\n") || "- No papers completed yet."}
