...
function getState() {
    return loadJSON("planner-state.json", DEFAULT_STATE);
}
...
export function markProjectMilestone(projectId, milestoneId) {
    const state = getState();
...
export function logStudySession(hours, topic, notes) {
    const state = getState();
...
export function overridePhase(phaseId) {
    const state = getState();
...
export function getFullRoadmap() {
