...
function getConfig() {
    return loadJSON("notify-config.json", DEFAULT_CONFIG);
}
...
export async function sendNotification(title, message, options) {
    ...
    try {
        ...
        // Log notification
        const log = loadJSON("notification-log.json", []);
        log.push({
            title,
...
export async function notifyStudyReminder(topic, startTime, minutesBefore = 10) {
    return sendNotification(`📚 Study Session in ${minutesBefore}min`, `Time to study: ${topic}\nStarting at ${startTime}\n\nConsistency > Intensity. Let's go!`, { priority: 4, type: "reminder" });
}
export async function notifyStreakUpdate(currentStreak, totalHours) {
    const messages = [
    ...
    return sendNotification(`🔥 ${currentStreak}-Day Study Streak!`, messages[currentStreak % messages.length], { priority: 3, type: "streak" });
}
export async function notifyRevisionDue(dueCount, topConcepts) {
    return sendNotification(`🧠 ${dueCount} Reviews Due Today`, `Spaced repetition items ready:\n${topConcepts.slice(0, 3).map((c) => `• ${c}`).join("\n")}\n${dueCount > 3 ? `...and ${dueCount - 3} more` : ""}`, { priority: 3, type: "revision" });
}
export async function notifyMilestoneComplete(project, milestone) {
    return sendNotification(`🏆 Milestone Complete!`, `${project}: "${milestone}"\n\nAnother step toward the Lotfollahi lab. Celebrate this win! 🎉`, { priority: 4, type: "milestone" });
}
export async function notifyWeeklySummary(summary) {
    return sendNotification(`📊 Weekly Study Summary`, `Hours: ${summary.hours}h\nStreak: ${summary.streak} days\nTopics: ${summary.topicsCompleted} completed\nPapers: ${summary.papersRead} read\n\nKeep building momentum! 💪`, { priority: 3, type: "study" });
}
// --- Configuration ---
export function configureNotifications(updates) {
    const config = getConfig();
...
export function testNotification() {
    return sendNotification("🧪 Test Notification", "Study Companion MCP is connected! You'll receive study reminders, streak updates, and milestone celebrations here.", { priority: 3, tags: ["white_check_mark"], bypassQuietHours: true });
}
