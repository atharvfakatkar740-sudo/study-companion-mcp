...
function loadPublishingStrategy() {
...
function loadPublicationProgress() {
    return loadJSON("publication-progress.json", {
...
function savePublicationProgress(progress) {
    saveJSON("publication-progress.json", progress);
}
// ============================================
// 1. Conference Tracking & Deadlines
// ============================================
export function getConferenceDeadlines(tier, withinDays) {
    ...
    for (const conf of conferences) {
        for (const [deadlineType, dateStr] of Object.entries(conf.deadlines)) {
            ...
            if (daysUntil < -30)
                continue; // skip long-past deadlines
            if (cutoff && deadline > cutoff)
                continue;
            upcoming.push({
...
export function getConferenceInfo(conferenceId) {
    const data = loadConferences();
...
export function getConferencesByTier() {
    const data = loadConferences();
    const tiers = { S: [], A: [], B: [] };
...
// ============================================
export function getPublicationStrategy() {
    const data = loadConferences();
    ...
    const masteredConcepts = Object.values(mastery).filter((m) => m.level === "advanced" || m.level === "expert").length;
...
function getPublishingNextActions(phase, progress, contributions) {
    const actions = [];
    if (phase === "phase1_0_6_months") {
        ...
        if (contributions === 0)
            actions.push("Make your first open-source contribution to scverse/scanpy");
    }
        ...
        if (progress.papers.length === 0)
            actions.push("Start drafting your first paper — use 'start_paper_draft'");
...
// ============================================
export function startPaperDraft(title, strategy, targetVenue, coauthors = [], notes = "") {
    const progress = loadPublicationProgress();
...
export function updatePaperDraft(draftId, updates) {
    const progress = loadPublicationProgress();
    const draft = progress.papers.find((p) => p.id === draftId);
    if (!draft)
        return { error: `Draft "${draftId}" not found.` };
    if (updates.status)
        draft.status = updates.status;
    if (updates.title)
        draft.title = updates.title;
    if (updates.targetVenue)
        draft.targetVenue = updates.targetVenue;
    if (updates.coauthors)
        draft.coauthors = updates.coauthors;
    if (updates.notes)
...
export function getPaperDrafts() {
    const progress = loadPublicationProgress();
    return {
...
// ============================================
export function getTargetResearchers(alignment) {
    const strategy = loadPublishingStrategy();
    let researchers = strategy.target_researchers;
    if (alignment) {
        researchers = researchers.filter((r) => r.alignment === alignment || r.alignment.includes(alignment));
    }
    return {
...
export function checkOutreachReadiness(researcherId) {
    const strategy = loadPublishingStrategy();
...
// ============================================
export function logEcosystemContribution(ecosystem, type, repo, description, url) {
    const progress = loadPublicationProgress();
    const today = new Date().toISOString().split("T")[0];
    const contribution = {
        ecosystem,
...
export function getEcosystemVisibility() {
    const strategy = loadPublishingStrategy();
    const progress = loadPublicationProgress();
...
function getEcosystemRecommendations(stats) {
    const recs = [];
...
// ============================================
export function getConferenceAlerts() {
    const data = loadConferences();
    const now = new Date();
    const alerts = [];
    for (const conf of data.conferences) {
...
// ============================================
export function getFirstPaperAdvice() {
    const data = loadConferences();
    const plannerState = loadJSON("planner-state.json", {});
    const mastery = loadJSON("mastery-state.json", {});
    const papers = loadJSON("papers.json", []);
    ...
    strategies.sort((a, b) => (viabilityOrder[a.viability] || 2) - (viabilityOrder[b.viability] || 2));
    return {
        title: "First Paper Strategy Advisor",
        yourStats: {
            papersRead: completedPapers.length,
            totalStudyHours: totalHours,
            masteredConcepts: Object.values(mastery).filter((m) => m.level === "advanced").length,
        },
        strategies,
...
export function getPublishingDashboard() {
    ...
    const nextDeadlines = confs.conferences
        .flatMap((c) => Object.entries(c.deadlines).map(([type, date]) => ({
        conference: c.name,
        tier: c.tier,
        type,
        date,
