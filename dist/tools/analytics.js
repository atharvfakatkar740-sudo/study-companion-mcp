...
export function getWeeklyAnalytics(weeksBack = 4) {
...
export function getTimeDistribution() {
    const sessions = loadSessionLog();
    const byTopic = {};
    const byDayOfWeek = { Mon: 0, Tue: 0, Wed: 0, Thu: 0, Fri: 0, Sat: 0, Sun: 0 };
...
export function getPredictedCompletion() {
    const sessions = loadSessionLog();
    const plannerState = loadJSON("planner-state.json", { startDate: new Date().toISOString().split("T")[0], completedTopics: [], completedProjects: [], totalStudyHours: 0 });
...
function generatePaceRecommendations(weeklyHours, velocity, weeksElapsed) {
    const recs = [];
    if (weeklyHours < 15) {
