...
export function dailyStandup() {
    ...
    const plannerState = loadJSON("planner-state.json", {
    ...
    const mastery = loadJSON("mastery-state.json", {});
    ...
    const readingPaper = papers.find((p) => p.status === "reading");
    ...
    const struggles = Object.entries(mastery)
        .filter(([, info]) => info.level === "beginner" && info.assessments >= 3)
        .map(([id, info]) => ({ concept: id, score: Math.round(info.score) }));
    // Streak info
...
export function endOfDayReview() {
    ...
    const todayAssessments = Object.entries(mastery)
        .filter(([, info]) => info.lastAssessed === today)
        .map(([id, info]) => ({ concept: id, level: info.level, score: Math.round(info.score) }));
    // What didn't happen
    const missedItems = [];
    const dueRevisions = revisions.filter((r) => r.nextReview <= today && r.lastReviewed !== today);
    if (dueRevisions.length > 0)
        missedItems.push(`${dueRevisions.length} SRS items still due`);
    if (todayHours === 0)
        missedItems.push("No study sessions logged");
    if (todayInsights.length === 0 && todayHours > 0)
        missedItems.push("No insights saved — capture key takeaways!");
    // Quality score
    let qualityScore = 50;
    if (todayHours > 0)
        qualityScore += 15;
...
function getVelocityRecommendations(status, hoursPerWeek, topicVelocity, phases) {
    const recs = [];
    if (status === "behind") {
        recs.push("You're behind schedule. Consider increasing weekly study hours or reducing scope.");
        const behindPhases = phases.filter((p) => p.status === "behind");
        if (behindPhases.length > 0) {
            recs.push(`Overdue phases: ${behindPhases.map((p) => p.name).join(", ")}. Prioritize completing these.`);
        }
    }
...
export function weeklyRetrospective() {
    ...
    const memories = loadJSON("memory.json", []);
    const mastery = loadJSON("mastery-state.json", {});
    const testResults = loadJSON("test-results.json", []);
    // This week
    ...
    const weekMastery = Object.entries(mastery)
        .filter(([, info]) => info.lastAssessed >= weekAgo)
        .map(([id, info]) => ({
        concept: id,
        level: info.level,
    ...
    // Tests this week
    const weekTests = testResults.filter((t) => t.date >= weekAgo);
    // Hours trend
    ...
    return {
        ...
        improvements: generateImprovements(thisWeekHours, activeDays, forgotCount, weekRevisions.length, weekInsights.length, topicDiversity, hoursTrend),
        nextWeekFocus: generateNextWeekFocus(mastery, revisions, phases),
    };
}
function generateWins(hours, activeDays, revisions, insights, tests) {
    const wins = [];
    if (hours >= 15)
        wins.push(`Strong study week: ${Math.round(hours)}h logged`);
    if (activeDays >= 6)
        wins.push(`${activeDays}/7 active days — excellent consistency`);
    if (revisions >= 20)
        wins.push(`${revisions} SRS reviews completed — retention is building`);
    if (insights >= 5)
        wins.push(`${insights} insights saved — active learning in action`);
    if (tests.length > 0) {
        const avgScore = tests.reduce((s, t) => s + (t.averageScore || 0), 0) / tests.length;
        if (avgScore >= 70)
            wins.push(`Test average: ${Math.round(avgScore)}% — solid understanding`);
    }
    if (wins.length === 0)
        wins.push("Every session counts. You showed up this week.");
    return wins;
}
function generateImprovements(hours, activeDays, forgot, revisions, insights, topicDiversity, hoursTrend) {
    const improvements = [];
    if (hours < 10)
        improvements.push("Aim for 10+ hours next week. Even small increases compound.");
    if (activeDays < 5)
        improvements.push("Study 5+ days per week. Consistency beats intensity.");
    if (forgot > revisions * 0.3 && revisions > 0)
        improvements.push(`High forget rate (${forgot}/${revisions}). Review prerequisites for struggling concepts.`);
    if (insights === 0)
        improvements.push("Save at least 1 insight per session — it builds your knowledge base.");
    if (topicDiversity < 50)
        improvements.push("Low topic diversity. Interleave topics for better retention.");
    if (hoursTrend === "decreasing")
        improvements.push("Hours trending down. Set a minimum daily commitment.");
    return improvements;
}
function generateNextWeekFocus(mastery, revisions, phases) {
    const focus = [];
    // Concepts needing attention
    const weakConcepts = Object.entries(mastery)
        .filter(([, info]) => info.level === "beginner" && info.assessments >= 2)
    ...
    if (overdue > 5)
        focus.push(`Clear ${overdue} overdue SRS items`);
...
export function detectBurnout() {
    ...
    const decliningConcepts = Object.entries(mastery)
        ...
        if (info.history && info.history.length >= 4) {
            const recent = info.history.slice(-2).map((h) => h.score);
            const earlier = info.history.slice(-4, -2).map((h) => h.score);
...
// --- Helper ---
function getCurrentPhaseFromState(state, phases) {
