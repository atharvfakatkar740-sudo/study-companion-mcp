...
function loadMastery() {
    return loadJSON("mastery-state.json", {});
}
...
// --- Public API ---
export function assessUnderstanding(conceptId, assessmentType, score, difficulty = "medium", timeSpentMinutes, notes) {
    ...
    const typeMultiplier = assessmentType === "derivation" ? 1.4
...
export function getDifficultyProfile(topic) {
    const mastery = loadMastery();
...
function getProfileRecommendation(levels, avgScore) {
...
export function generateChallenge(conceptId, challengeType) {
    const mastery = loadMastery();
...
function selectChallengeType(level) {
    if (level === "advanced")
        return "derivation";
...
function buildChallenge(name, id, type, difficulty) {
    switch (type) {
        case "derivation":
            return {
...
export function detectStruggle(conceptId) {
    const mastery = loadMastery();
    ...
    for (const [id, info] of entries) {
        ...
        if (info.lastAssessed) {
            const daysSince = Math.floor((new Date(today).getTime() - new Date(info.lastAssessed).getTime()) / (24 * 60 * 60 * 1000));
        ...
        const conceptRevisions = revisions.filter((r) => r.concept.toLowerCase().includes(id.replace(/-/g, " ").toLowerCase()));
        const forgotCount = conceptRevisions.filter((r) => r.difficulty === "forgot").length;
        if (forgotCount >= 3) {
            struggles.push({
                conceptId: id,
...
export function getMasteryDashboard() {
    const mastery = loadMastery();
    const revisions = loadJSON("revisions.json", []);
    const entries = Object.entries(mastery);
    if (entries.length === 0) {
