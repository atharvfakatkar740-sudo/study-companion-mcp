...
export async function fetchGitHubActivity(username) {
    try {
        ...
        const relevantRepos = repoList.filter((r) => r.name.toLowerCase().includes("vae") ||
...
export async function searchArxiv(query, maxResults = 10) {
    try {
        ...
        const relevant = papers.filter((p) => p.title.toLowerCase().includes("single-cell") ||
...
export async function fetchResearcherPapers(researcherName) {
    const config = getResearcherConfig();
    if (researcherName) {
        const researcher = config.researchers.find((r) => r.name.toLowerCase().includes(researcherName.toLowerCase()));
...
export async function fetchLotfollahiPapers() {
    return fetchResearcherPapers("Lotfollahi");
}
// --- Blog Post Drafting ---
export function draftBlogPost(topic, style = "concept-explanation") {
    const memories = loadJSON("memory.json", []);
    const topicMemories = memories.filter((m) => m.topic.toLowerCase().includes(topic.toLowerCase()) ||
