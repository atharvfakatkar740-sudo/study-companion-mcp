...
import { storeChatMessage, searchChatHistory, getRelevantContext, } from "./vectordb.js";
...
function getOrCreateSession() {
    if (!currentSessionId) {
        ...
        sessionMessages = [];
        // Track sessions
        const sessions = loadJSON("chat-sessions.json", []);
        sessions.push({
            id: currentSessionId,
...
function updateSessionMeta(topics = []) {
    if (!currentSessionId)
        return;
    const sessions = loadJSON("chat-sessions.json", []);
    ...
    if (session) {
        ...
        for (const t of topics) {
            if (!session.topics.includes(t))
                session.topics.push(t);
        }
        saveJSON("chat-sessions.json", sessions);
    }
}
// --- Main Chat Function ---
export async function studyChat(message, options) {
...
// --- Context-Based Q&A ---
export async function askContextQuestion(question) {
...
export async function getChatSessions(limit = 10) {
    const sessions = loadJSON("chat-sessions.json", []);
...
export async function searchPastChats(query, limit = 10) {
    try {
