...
function loadCitationGraph() {
    return loadJSON("citation-graph.json", {
        nodes: [], edges: [], lineages: [], lastUpdated: new Date().toISOString(),
    });
}
function saveCitationGraph(graph) {
    graph.lastUpdated = new Date().toISOString();
    saveJSON("citation-graph.json", graph);
}
function loadChecklists() {
    return loadJSON("implementation-checklists.json", []);
}
...
async function fetchFromSemanticScholar(endpoint) {
    try {
...
export async function searchSemanticScholar(query, limit = 10) {
    const data = await fetchFromSemanticScholar(`/paper/search?query=${encodeURIComponent(query)}&limit=${limit}&fields=${S2_FIELDS}`);
    if (data.error)
        return data;
...
// ============================================
export async function addToCitationGraph(paperId, fetchCitations = true, depth = 1) {
    const graph = loadCitationGraph();
    // Fetch paper details from Semantic Scholar
    const data = await fetchFromSemanticScholar(`/paper/${paperId}?fields=${S2_FIELDS}`);
    if (data.error)
        return data;
        ...
        for (const cite of cites.slice(0, 15)) {
            ...
            if (!graph.nodes.find((n) => n.paperId === cite.paperId)) {
                graph.nodes.push({
                    ...
                    authors: (cite.authors || []).map((a) => a.name),
                    year: cite.year,
                    venue: cite.venue,
...
export function addManualCitation(fromTitle, toTitle, type = "cites") {
    const graph = loadCitationGraph();
    ...
    if (!fromNode || !toNode)
        return { error: "Could not find or create paper nodes." };
    const edgeExists = graph.edges.find((e) => e.from === fromNode.paperId && e.to === toNode.paperId);
    if (edgeExists)
        return { message: "Edge already exists.", edge: edgeExists };
...
export function getCitationGraph(paperTitle) {
    const graph = loadCitationGraph();
    if (paperTitle) {
        const node = graph.nodes.find((n) => n.title.toLowerCase().includes(paperTitle.toLowerCase()));
        if (!node)
            return { error: `Paper "${paperTitle}" not found in citation graph.` };
...
// ============================================
export function trackPaperLineage(lineageName, description, papers) {
    const graph = loadCitationGraph();
...
export function getPaperLineages() {
    const graph = loadCitationGraph();
...
// ============================================
export function generateImplementationChecklist(paperTitle, paperType = "general") {
...
function getChecklistTemplate(type, title) {
    const common = [
        {
...
export function updateChecklistItem(checklistId, itemId, completed, notes) {
    const checklists = loadChecklists();
    const checklist = checklists.find((c) => c.id === checklistId);
    if (!checklist)
        return { error: `Checklist "${checklistId}" not found.` };
    let found = false;
    ...
    if (!found)
        return { error: `Item "${itemId}" not found in checklist.` };
    // Update checklist status
    const totalItems = checklist.sections.reduce((s, sec) => s + sec.items.length, 0);
    const completedItems = checklist.sections.reduce((s, sec) => s + sec.items.filter((i) => i.completed).length, 0);
...
export function getImplementationChecklists(checklistId) {
    const checklists = loadChecklists();
    if (checklistId) {
        const checklist = checklists.find((c) => c.id === checklistId);
        if (!checklist)
    ...
    return {
        ...
        checklists: checklists.map((c) => {
            ...
            const completedItems = c.sections.reduce((s, sec) => s + sec.items.filter((i) => i.completed).length, 0);
            return {
                id: c.id,
...
// ============================================
export function findBridgePapers(topicA, topicB) {
    const graph = loadCitationGraph();
    // Find papers that have both topics in their title/venue or are connected to papers in both topics
    const papersA = graph.nodes.filter((n) => n.title.toLowerCase().includes(topicA.toLowerCase()) ||
        n.venue?.toLowerCase().includes(topicA.toLowerCase()));
    const papersB = graph.nodes.filter((n) => n.title.toLowerCase().includes(topicB.toLowerCase()) ||
        n.venue?.toLowerCase().includes(topicB.toLowerCase()));
    // Direct bridges: papers that match both topics
    const directBridges = graph.nodes.filter((n) => (n.title.toLowerCase().includes(topicA.toLowerCase()) ||
        n.venue?.toLowerCase().includes(topicA.toLowerCase())) &&
        (n.title.toLowerCase().includes(topicB.toLowerCase()) ||
            n.venue?.toLowerCase().includes(topicB.toLowerCase())));
    // Indirect bridges: papers cited by both topic groups
    const aReferences = new Set(papersA.flatMap((p) => graph.edges.filter((e) => e.from === p.paperId).map((e) => e.to)));
    const bReferences = new Set(papersB.flatMap((p) => graph.edges.filter((e) => e.from === p.paperId).map((e) => e.to)));
    ...
    return {
        ...
        indirectBridges: indirectBridges.map((n) => ({ title: n.title, year: n.year, citations: n.citationCount })),
...
// ============================================
export function citationGraphStats() {
    const graph = loadCitationGraph();
    ...
    for (const n of graph.nodes) {
        if (n.year)
            yearDist[n.year] = (yearDist[n.year] || 0) + 1;
    }
    const venueDist = {};
    for (const n of graph.nodes) {
        if (n.venue)
            venueDist[n.venue] = (venueDist[n.venue] || 0) + 1;
    }
    const edgeTypeDist = {};
...
//# sourceMappingURL=research-intel.js.map