...
function getPapers() {
    ...
    if (papers.length === 0) {
        ...
        const initial = readingList.map((p, i) => ({
            id: `paper-${i + 1}`,
            title: p.title,
            authors: [p.authors],
            url: p.url,
...
export function addPaper(title, authors, url, phase, priority) {
...
export function startReadingPaper(paperId) {
    const papers = getPapers();
...
export function annotatePaper(paperId, notes, keyInsights, architectureNotes) {
    const papers = getPapers();
    const paper = papers.find((p) => p.id === paperId);
    if (!paper)
        return { error: `Paper '${paperId}' not found.` };
    if (notes)
        paper.notes = notes;
    if (keyInsights)
        paper.keyInsights = keyInsights;
    if (architectureNotes)
        paper.architectureNotes = architectureNotes;
    saveJSON("papers.json", papers);
    return { success: true, message: `Annotations saved for "${paper.title}".` };
}
export function completePaper(paperId, keyInsights) {
    const papers = getPapers();
    const paper = papers.find((p) => p.id === paperId);
    if (!paper)
        return { error: `Paper '${paperId}' not found.` };
...
export function getPaperNotes(paperId) {
    const papers = getPapers();
    const paper = papers.find((p) => p.id === paperId);
    if (!paper)
        return { error: `Paper '${paperId}' not found.` };
...
export function searchPapers(query) {
    ...
    const results = papers.filter((p) => p.title.toLowerCase().includes(q) ||
