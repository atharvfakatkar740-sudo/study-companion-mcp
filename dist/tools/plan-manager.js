import { getStudyPlan, getStudyPhases, getPlanMeta, savePlan, snapshotPlan, getPlanHistory, reloadPlan, getResearcherConfig, } from "../engine/plan-loader.js";
// ============================================
...
export function reloadStudyPlan() {
    try {
        reloadPlan();
...
export function validatePlan() {
    try {
        ...
        if (dupTopics.length > 0)
            warnings.push(`Duplicate topic IDs: ${dupTopics.join(", ")}`);
        ...
        if (dupProjects.length > 0)
            warnings.push(`Duplicate project IDs: ${dupProjects.join(", ")}`);
...
export function addPhaseToPlan(id, name, monthStart, monthEnd, description) {
    try {
        const plan = getStudyPlan();
...
export function addTopicToPlan(phaseId, topicId, name, priority, subtopics) {
    try {
        const plan = getStudyPlan();
        const phase = plan.phases.find((p) => p.id === phaseId);
        if (!phase)
            return { error: `Phase '${phaseId}' not found.` };
...
export function addProjectToPlan(phaseId, projectId, name, description, milestones) {
    try {
        const plan = getStudyPlan();
        const phase = plan.phases.find((p) => p.id === phaseId);
        if (!phase)
...
export function snapshotCurrentPlan(label) {
    try {
...
export function listPlanSnapshots() {
...
export function getResearchersInfo() {
