...
import { findPhaseById, getCurrentPhaseFromPlan } from "../engine/plan-loader.js";
...
const DEFAULT_WEEKEND_TEMPLATE = [
...
function getScheduleConfig() {
    return loadJSON("schedule-config.json", {
        weekday: DEFAULT_WEEKDAY_TEMPLATE,
        weekend: DEFAULT_WEEKEND_TEMPLATE,
    });
}
export function scheduleStudySessions(startDate, daysAhead = 7, customTopics) {
    const config = getScheduleConfig();
    const plannerState = loadJSON("planner-state.json", { startDate: new Date().toISOString().split("T")[0], completedTopics: [] });
    ...
    const hasDueRevisions = revisions.some((r) => r.nextReview <= today);
    const sessions = [];
    ...
    for (let day = 0; day < daysAhead; day++) {
        ...
        for (const slot of template) {
            if (slot.activity === "Break" || slot.activity === "Lunch Break")
                continue;
...
export function updateScheduleTemplate(dayType, slots) {
    const config = getScheduleConfig();
...
export function getNextStudySession() {
    const sessions = loadJSON("scheduled-sessions.json", []);
...
export function createCalendarEvent(title, date, startHour, durationMinutes, description) {
...
export function getWeeklyScheduleOverview() {
    const config = getScheduleConfig();
...
// --- Helpers ---
function getCurrentPhaseFromDate(startDate) {
