...
function getConfig() {
    return loadJSON("ollama-config.json", DEFAULT_CONFIG);
}
// --- Core API Calls ---
...
export async function ollamaEmbed(texts, model) {
    const config = getConfig();
...
export async function checkOllamaStatus() {
    ...
    try {
        ...
        const hasChat = models.some((m) => m.name.includes(config.chatModel.split(":")[0]));
        const hasEmbed = models.some((m) => m.name.includes(config.embedModel.split(":")[0]));
        return {
            status: "connected",
...
export function configureOllama(updates) {
    const config = getConfig();
...
export function getStudyCompanionSystem() {
