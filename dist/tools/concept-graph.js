...
function loadGraph() {
    return loadJSON("concept-graph.json", { nodes: [], edges: [] });
}
function saveGraph(graph) {
    saveJSON("concept-graph.json", graph);
}
// --- Public API ---
export function addConcept(id, name, topic, description) {
...
export function addConceptDependency(prerequisiteId, dependentId, strength = "required") {
    const graph = loadGraph();
...
function wouldCreateCycle(graph, from, to) {
    ...
    while (queue.length > 0) {
        ...
        if (current === to)
            return false; // 'to' reaching 'from' is fine, we need 'from' reachable from 'to'
        visited.add(current);
    }
    ...
    while (queue2.length > 0) {
        ...
        if (current === from)
            return true; // 'to' can reach 'from', so adding from→to makes a cycle
...
export function getLearningPath(targetConceptId) {
    const graph = loadGraph();
    const node = graph.nodes.find((n) => n.id === targetConceptId);
    if (!node)
        return { error: `Concept '${targetConceptId}' not found.` };
    ...
    const completedTopics = loadJSON("planner-state.json", { completedTopics: [] }).completedTopics;
    const path = sorted.map((id, index) => {
        const concept = graph.nodes.find((n) => n.id === id);
        const masteryLevel = mastery[id]?.level || "unknown";
        const hasRevision = revisions.some((r) => r.concept.toLowerCase().includes(id.replace(/-/g, " ").toLowerCase()));
        return {
            step: index + 1,
            conceptId: id,
    ...
    path.push({
        ...
        inSRS: revisions.some((r) => r.concept.toLowerCase().includes(targetConceptId.replace(/-/g, " ").toLowerCase())),
        prerequisites: graph.edges
...
function getAllPrerequisites(graph, conceptId) {
...
export function visualizeKnowledgeGraph(topic) {
    const graph = loadGraph();
    const mastery = loadJSON("mastery-state.json", {});
    ...
    if (topic) {
        ...
        filteredNodes = graph.nodes.filter((n) => n.topic?.toLowerCase().includes(topicLower) || n.name.toLowerCase().includes(topicLower));
...
function buildAsciiTree(graph, mastery, nodeId, lines, prefix, isLast, visited) {
...
export function suggestNextConcept() {
    ...
    for (const node of graph.nodes) {
        ...
        const hasSRSDue = revisions.some((r) => r.concept.toLowerCase().includes(node.id.replace(/-/g, " ").toLowerCase()) && r.nextReview <= today);
        let score = dependentCount * 10;
        if (level === "unknown")
            score += 5; // Unstarted concepts get a boost
        if (hasSRSDue)
            score += 3; // Has pending review
        if (prereqs.length === 0)
            score -= 2; // Foundational (already easy)
...
export function getGraphStats() {
    ...
    const mastery = loadJSON("mastery-state.json", {});
    const byTopic = {};
    const byMastery = { unknown: 0, beginner: 0, intermediate: 0, advanced: 0 };
    for (const node of graph.nodes) {
