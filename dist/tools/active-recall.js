...
// --- Derivation Quiz ---
export function generateDerivationQuiz(topic, count = 3) {
    ...
    const relatedRevisions = revisions.filter((r) => r.topic.toLowerCase().includes(topicLower) ||
        r.concept.toLowerCase().includes(topicLower));
    const relatedMemories = memories.filter((m) => m.topic.toLowerCase().includes(topicLower) ||
...
function generateDerivationPrompt(concept, difficulty) {
    const hard = [
...
export function generateImplementationChallenge(topic, language = "python") {
    const mastery = loadJSON("mastery-state.json", {});
...
export function generateELI5Challenge(concept) {
...
// --- Compare & Contrast ---
export function generateComparison(conceptA, conceptB) {
...
export function generateWeeklyTest() {
    const revisions = loadJSON("revisions.json", []);
    const memories = loadJSON("memory.json", []);
    const mastery = loadJSON("mastery-state.json", {});
    const sessions = loadJSON("session-log.json", []);
    ...
    const weakest = Object.entries(mastery)
        .filter(([, info]) => info.level === "beginner" || (info.level === "intermediate" && info.score < 60))
    ...
    const totalTime = sections.reduce((s, sec) => s + sec.questions.reduce((qs, q) => qs + (q.timeLimit || 5), 0), 0);
...
// --- Anki Export ---
export function exportAnkiDeck(topic, includeInsights = true) {
    const revisions = loadJSON("revisions.json", []);
    const memories = loadJSON("memory.json", []);
    const mastery = loadJSON("mastery-state.json", {});
    ...
    if (topic) {
        const topicLower = topic.toLowerCase();
        srsItems = srsItems.filter((r) => r.topic.toLowerCase().includes(topicLower) || r.concept.toLowerCase().includes(topicLower));
        insightItems = insightItems.filter((m) => m.topic.toLowerCase().includes(topicLower) || m.content.toLowerCase().includes(topicLower));
    }
    ...
    const tsvLines = cards.map((c) => `${escapeAnki(c.front)}\t${escapeAnki(c.back)}\t${c.tags}`);
...
// --- Helpers ---
function findMasteryLevel(mastery, concept) {
    // Direct match
    const conceptId = concept.toLowerCase().replace(/\s+/g, "-");
    if (mastery[conceptId])
        return mastery[conceptId].level;
    // Fuzzy match
    const entry = Object.entries(mastery).find(([id]) => id.includes(conceptId) || conceptId.includes(id));
    return entry ? entry[1].level : "unknown";
}
export function recordTestResult(testId, scores, duration) {
    const results = loadJSON("test-results.json", []);
